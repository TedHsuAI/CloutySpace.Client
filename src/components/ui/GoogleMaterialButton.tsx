...
interface GoogleMaterialButtonProps {
  lang: Language
  onClick?: () => void
  disabled?: boolean
  className?: string
...
const GoogleMaterialButton: FC<GoogleMaterialButtonProps> = ({ lang, onClick, disabled, className = '' }) => {
  const text = i18n[lang].loginWithGoogle
  return (
  <button className={`gsi-material-button ${className}`} onClick={onClick} disabled={disabled} type="button">
      ...
      <style>{`
        ...
        .gsi-material-button-content-wrapper{display:flex;align-items:center;width:100%;}
        .gsi-material-button-contents{font-weight:500;flex-grow:1;text-align:center;}
        .gsi-material-button-state{position:absolute;inset:0;opacity:0;transition:opacity .218s;pointer-events:none;}
      `}</style>
    </button>
  )
}
